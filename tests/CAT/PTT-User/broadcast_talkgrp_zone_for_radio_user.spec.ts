...
test.describe('One Portal CAT - Broadcast Talkgroup Zone', () => {
  ...
  test('TC_050 Create Broadcast Talk Group with assigning the Radio type User', async ({ page }) => {
  ...
  test('TC_051 Verify zone(N/A) for Broadcast Talk Group of Radio User', async ({ page }) => {
    await wcsrLogin(page);
    pageObj = new CATPage(page);
  ...
  test('TC_052 Delete talkgroup', async ({ page }) => {
    await wcsrLogin(page);
    pageObj = new CATPage(page);
