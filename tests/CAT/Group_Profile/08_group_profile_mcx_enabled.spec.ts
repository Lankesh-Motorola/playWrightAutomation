...
test.describe('One Portal CAT - Group Profile MCX Enabled', () => {
  ...
  test('GP-049 Verify Creating Dispatch group profile with MCX enabled', async ({ page, context }) => {
  ...
  test('GP-050 Verify Creating Broadcast group profile with MCX enabled', async ({ page, context }) => {
    await wcsrLogin(page);
    pageObj = new CATPage(page);
    // Launch and get the new page object
    const newPageObj = await launchAndGetNewPageObject(context, pageObj, corp_id);
